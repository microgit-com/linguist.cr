    ...
    class Classifier < Cadmium::Classifier::Bayes
      ...
      def tokens_probability(tokens, language)
        tokens.reduce(0.0) do |sum, token|
          next sum unless @word_frequency_count[language.name]?
          sum += Math.log(token_probability(token, language.name))
        end
      end

      def language_probability(language)
        return 0.0 if language.nil?
        return 0.0 unless language.is_a?(Language)
        Math.log((@word_count[language.name]? || 0.0).to_f / @categories.size.to_f)
      end
